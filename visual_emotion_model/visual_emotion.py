    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic, \
        ...
        while cap.isOpened():
            ...
            if face_results.multi_face_landmarks:
                ...
                left_eye_points = np.array([[int(p.x * img_w), int(p.y * img_h)] for idx, p in enumerate(face_landmarks.landmark) if idx in LEFT_EYE])
                left_midpoint = np.mean(left_eye_points, axis=0)

                # Calculate right eye midpoint
                right_eye_points = np.array([[int(p.x * img_w), int(p.y * img_h)] for idx, p in enumerate(face_landmarks.landmark) if idx in RIGHT_EYE])
                right_midpoint = np.mean(right_eye_points, axis=0)

                # Calculate iris centers
                left_iris_center = np.mean(np.array([[int(p.x * img_w), int(p.y * img_h)] for idx, p in enumerate(face_landmarks.landmark) if idx in LEFT_IRIS]), axis=0)
                right_iris_center = np.mean(np.array([[int(p.x * img_w), int(p.y * img_h)] for idx, p in enumerate(face_landmarks.landmark) if idx in RIGHT_IRIS]), axis=0)
                ...
                lip_width = math.sqrt((abs(face_landmarks.landmark[287].y - face_landmarks.landmark[57].y))**2 + (abs(face_landmarks.landmark[287].x - face_landmarks.landmark[57].x))**2)
                jaw_width = math.sqrt((abs(face_landmarks.landmark[401].y - face_landmarks.landmark[177].y))**2 + (abs(face_landmarks.landmark[401].x - face_landmarks.landmark[177].x))**2)
                
                smile_ratio = lip_width/jaw_width
                #print(smile_ratio)
                
                lip_left = math.sqrt((abs(face_landmarks.landmark[287].y - face_landmarks.landmark[0].y))**2 + (abs(face_landmarks.landmark[287].x - face_landmarks.landmark[0].x))**2)
                lip_right = math.sqrt((abs(face_landmarks.landmark[0].y - face_landmarks.landmark[57].y))**2 + (abs(face_landmarks.landmark[0].x - face_landmarks.landmark[57].x))**2)
                lip_semi = (lip_right + lip_left + lip_width)/2
                lip_area = math.sqrt(lip_semi*(lip_semi - lip_right)*(lip_semi - lip_left)*(lip_semi - lip_width))
                ...
                if lip_height < 0.01 and smile_ratio > 0.49:
                    smiling = True
                    outSmiling = ""
                    cv2.putText(image, 'Smiling', (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                else:
                    smiling = False
                    outSmiling = "not "
                ...
                brow_width = math.sqrt((abs(face_landmarks.landmark[285].y - face_landmarks.landmark[55].y))**2 + (abs(face_landmarks.landmark[285].x - face_landmarks.landmark[55].x))**2)
                eye_width = math.sqrt((abs(face_landmarks.landmark[362].y - face_landmarks.landmark[133].y))**2 + (abs(face_landmarks.landmark[362].x - face_landmarks.landmark[133].x))**2)
                ...
                left_eyelid = math.sqrt((abs(face_landmarks.landmark[442].y - face_landmarks.landmark[295].y))**2 + (abs(face_landmarks.landmark[442].x - face_landmarks.landmark[295].x))**2)
                left_brow = math.sqrt((abs(face_landmarks.landmark[295].y - face_landmarks.landmark[296].y))**2 + (abs(face_landmarks.landmark[295].x - face_landmarks.landmark[296].x))**2)
                right_eyelid = math.sqrt((abs(face_landmarks.landmark[222].y - face_landmarks.landmark[65].y))**2 + (abs(face_landmarks.landmark[222].x - face_landmarks.landmark[65].x))**2)
                right_brow = math.sqrt((abs(face_landmarks.landmark[65].y - face_landmarks.landmark[66].y))**2 + (abs(face_landmarks.landmark[65].x - face_landmarks.landmark[66].x))**2)
                ...
                for face_landmarks in face_results.multi_face_landmarks:
                    ...
                    if profile == 11 or profile == -1:       
                        if not recording:
                            ...
                            oldhead_height = x
                            head_height = x
                        cv2.putText(image,"Nods:",(440,240),cv2.FONT_HERSHEY_SIMPLEX,2,(255,0,0),3)
                        cv2.putText(image,str(nods),(500,290),cv2.FONT_HERSHEY_SIMPLEX,2,(255,0,0),3)
                    ...
                    cv2.putText(image,text,(20,400),cv2.FONT_HERSHEY_SIMPLEX,2,(0,255,0),2)
            ...
            try:    
                if profile < 10 and profile > -1:
                    demographics = DeepFace.analyze(image, actions=['age', 'gender', 'race'], silent=True)
                    ...
                    cv2.putText(image, text, (300, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    ...
                    cv2.putText(image, text, (300, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    
                    outRace = races.most_common(1)[0][0]
                    text = f"Race: {races.most_common(1)[0][0]}"
                    cv2.putText(image, text, (300, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                
                emotion_results = DeepFace.analyze(image, actions=['emotion'], silent=True)
                ...
                cv2.putText(image, text, (20, 350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            ...
            try:
                ...
                shoulder_width = math.sqrt((abs(left_shoulder.y - right_shoulder.y))**2 + (abs(left_shoulder.x - right_shoulder.x))**2)
                ...
                if frameCounter == 0:
                    ...
                    if prev_left_shoulder != None and prev_right_shoulder != None:
                        ...
                        shoulder_growth = (shoulder_width - oldshoulder_width)/oldshoulder_width
                ...
                # Display Lean Status
                cv2.putText(image, 'LEANING: {}'.format(leanFlag), (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    ...
                    if results.left_hand_landmarks and results.right_hand_landmarks:
                        ...
                        for left_landmark in left_hand_landmarks.landmark:
                            ...
                            for right_landmark in right_hand_landmarks.landmark:
                                ...
                                distance = ((left_x - right_x)**2 + (left_y - right_y)**2 + (left_z - right_z)**2)**0.5
                ...
                cv2.putText(image, 'OPEN POSE: {}'.format(openFlag), (10, 95), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
